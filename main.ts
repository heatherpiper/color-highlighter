...
export default class ColorHighlighterPlugin extends Plugin {
    ...
    blendRgbaWithBackground(rgba: string, background: string): string {
        try {
            ...
            // If background is fully transparent, use white as default
            if (bgA === 0) {
    ...
    extractRgbComponents(colorString: string): [number, number, number, number] {
        ...
        if (colorString.startsWith('#')) {
            ...
            // Extract alpha if it's an hsla color
